  ...
  let rec string_of_k3ir ?(string_of_meta = (fun _ -> "")) i = match i with
    ...
    | Node(m, n, t) -> 
      ...
      (ListExtras.ocaml_of_list
         (string_of_k3ir ~string_of_meta:string_of_meta) t)
end
...
struct
    ...
    | Leaf(_, Undecorated(mape)) ->
      ...
      List.iter (fun x -> 
         if x >= (List.length schema) then failwith (
               ...
               (ListExtras.ocaml_of_list string_of_int indices)^" out of "^
               (ListExtras.ocaml_of_list fst schema)
            )
      ) indices;
  ...
  let tuple_ir metadata sub_ir =
    build_code sub_ir
      (fun fieldse cir ->
        let e = K3.Tuple(List.map snd fieldse)
  ...
  let project_ir metadata projections sub_ir =
    build_code sub_ir
      (fun tu cir -> match tu with
        | [sym, te] ->
          let e = K3.Project(te, projections)
          in tag_of_undecorated metadata e cir
  ...
  let block_ir metadata sub_ir =
    build_code sub_ir
      (fun ucl cir ->
        let e = K3.Block (List.map snd ucl)
        in tag_of_undecorated metadata e cir)
  ...
  let iterate_ir metadata sub_ir =
    build_code sub_ir
      (fun fcu cir -> match fcu with
        | [(sym, fe); (_, ce)] ->
          let e = K3.Iterate(fe, ce) in tag_of_undecorated metadata e cir
  ...
  let lambda_ir metadata arg sub_ir =
    build_code sub_ir
      (fun bu cir -> match bu with
        | [(sym, be)] ->
          let e = K3.Lambda(arg, be) in tag_of_undecorated metadata e cir
  ...
  let assoc_lambda_ir metadata arg1 arg2 sub_ir =
    build_code sub_ir
      (fun bu cir -> match bu with
        | [(sym, be)] ->
          let e = K3.AssocLambda(arg1, arg2, be)
          in tag_of_undecorated metadata e cir
  ...
  let member_ir metadata sub_ir =
    build_code sub_ir
      (fun mku cir -> match mku with
        | mape::keyse ->
          let e = K3.Member(snd mape, List.map snd keyse)
          in tag_of_undecorated metadata e cir
  ...
  let lookup_ir metadata sub_ir = 
    build_code sub_ir
      (fun mku cir -> match mku with
        | mape::keyse ->
          let e = K3.Lookup(snd mape, List.map snd keyse)
          in tag_of_undecorated metadata e cir
...
struct
  ...
  let imp_of_ir (env : 'ext_type_t var_t list)
    ...
    and gc_expr e : ('a option, 'ext_type, 'ext_fn) expr_t =
      ...
      | K.SingletonPC(id,_) 
      | K.OutPC(id,_,_) | K3.InPC(id,_,_) | K3.PC(id,_,_,_) ->
    ...
    let rec gc_tag meta (t : node_tag_t) cmeta_l =
      ...
      let crv i =
        ...
        | Var(_,(id,ty)) ->
          begin match ty with
          | Host(K3.Fn(_,x)) -> mk_var id (Host x)
          | _ -> mk_var id ty
          end
