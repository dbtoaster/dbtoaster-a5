...
let tc_fn_app_suffix (rt_f : K.type_t list -> K.type_t) fn_tll sch_tl : K.type_t =
	...
	if valid_suffix then
		List.fold_left (fun acc_t tl -> K.Fn(tl, acc_t)) (rt_f rt) ret_fn_a_t
	else type_error (K.TTuple[K.TTuple(List.map (fun x -> K.TTuple(x)) fn_tll);
				K.TTuple(sch_tl)])
...
let tc_fn_app_each (rt_f : K.type_t list -> K.type_t) fn_tll t : K.type_t =
	...
	(* tc_fn_app_suffix rt_f fn_tll [t] *)
			type_error (K.TTuple[K.TTuple(List.map (fun x -> K.TTuple(x)) fn_tll); t])
...
let rec typecheck_expr e : K.type_t =
	try
				...
				begin match c_t with
					| K.TTuple(t_l) ->
							...
							if valid then
								...
								if (List.length idx) = 1 then List.hd ret_tl else K.TTuple(ret_tl)
		...
		(* TODO: almost the same as map typechecking... lift *)
				...
				let tc_iter_rt = tc_compose_collection (Some([K.TUnit])) (fun _ _ -> K.TUnit)
		...
		| K.Aggregate (fn_e, i_e, ce) ->
				tc_agg (recur fn_e) (recur i_e) (recur ce) (fun c_ret_t agg_tl ->
								...
								| (K.Collection _, _) -> failwith "invalid aggregate type"
		...
		| K.PCUpdate (me, ke, te) ->
				begin match me, ke with
					...
					| (K.PC(id, ins, outs, t(*,ie*)), _) ->
							...
							in
							if valid_tier then
								try ignore(tc_schema_exprs ins (List.map recur ke)); K.TUnit
