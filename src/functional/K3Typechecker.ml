...
(* safe to lift? collections of functions are generally not * safe for     *)
...
(* collection record (i.e. functions are data). * * Map lifting: * --      *)
(* map(f : Fn(b,Fn(c,d)), Lambda(g, h : C([b,c]))) * = Lambda(g,map(f,h))  *)
(* : Fn(g,d) * here Lambda(g,h) can be lifted since maps only apply to     *)
(* collections, * and there is a single function yielding the collection * *)
...
(* -- implicit lifting from collections, e.g. * FFn([w,x,y],               *)
...
let type_error (t: K3.SR.k3_type_t) (msg: string) = (
		Debug.print "K3-TYPECHECK-DETAIL" (fun () -> msg ^"\n\n"^ (K3.SR.string_of_type t));
		failwith msg
	)
...
let is_flat t = match t with | TBase(_) -> true | _ -> false
let flat t = if is_flat t then t else type_error t "invalid flat expression"

let promote t1 t2 =
	let open Types in
	match t1, t2 with
	| (TBase(TFloat), TBase(TInt)) | (TBase(TInt), TBase(TFloat)) -> TBase(TFloat)
...
let tc_fn_arg ae e : k3_type_t list =
	let aux (v, v_t) =
...
(* fn_tll, a linearized nested function type * -- sch_t, a collection      *)
...
let tc_fn_app_suffix (rt_f : k3_type_t list -> k3_type_t) fn_tll sch_tl : k3_type_t =
...
(* binding, i.e. * ++ if t : collection(tuple(x)), validates against x *   *)
(* ++ if t : collection(x), validates against x * -- yields a function     *)
(* type f : x+ -> coll_fn y, where fn_tl = [x+;y]                          *)
let tc_fn_app_each (rt_f : k3_type_t list -> k3_type_t) fn_tll t : k3_type_t =
	match t with
	| Collection(c_t) ->
...
let lift_fn (lift_f : k3_type_t list -> k3_type_t list list -> 'a)
		(fn_t : k3_type_t) : 'a =
...
(* another * function g, that yields a collection. * arguments: * --       *)
(* fn_tll, a linearized nested function f * -- fn_c_t, a function g, that  *)
(* yields a collection * notes: * -- lifts g above f, checking f can be    *)
...
(* (tc_rt_f c) [d])                                                        *)
let tc_compose_fn (crt_f : k3_type_t -> k3_type_t list -> k3_type_t)
		(fn_tll : k3_type_t list list) (fn_c_t : k3_type_t) =
...
(* implicitly lifts multivariate collection functions and * partial agg    *)
(* function application                                                    *)
let tc_agg fn_t i_t c_t (tc_gb_f : k3_type_t -> k3_type_t list -> k3_type_t) =
...
(* validates k = k_t * -- input m may be a fn, m : x+ -> collection        *)
...
let rec typecheck_expr e : k3_type_t =
	try
		let recur = typecheck_expr in
		match e with
		| Const c -> TBase(type_of_const c)
		...
		(* Persistent collections define records ,i.e. variables and their     *)
		(* types, * and may contain nested collections, e.g.                   *)
		...
		| Member (me, ke) -> tc_pcop (fun _ _ -> TBase(TFloat)) (recur me) (List.map recur ke)
