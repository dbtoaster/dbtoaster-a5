...
type stmt_info_t = string * K.expr_t list * K.expr_t * 
                   K.expr_t list * K.expr_t list * T.type_t * 
                   K.expr_t list * K.expr_t * K.expr_t

let is_ivc_constant = one_int_val
let is_gc_constant  = minus_one_int_val
let is_normal_constant = zero_int_val

let stmt_info_list : ((Plan.stmt_t, stmt_info_t) Hashtbl.t) = 
   Hashtbl.create 10

let get_stmt_info_from_dm_stmt (dm_stmt: Plan.stmt_t) : (stmt_info_t) = 
   Hashtbl.find stmt_info_list dm_stmt

let get_stmt_info (meta: meta_t) (dm_stmt: Plan.stmt_t) trig_args : 
                  (stmt_info_t * meta_t) = 
   let (mapn, lhs_ins, lhs_outs, map_type, init_calc_opt) = 
      Plan.expand_ds_name dm_stmt.Plan.target_map 
   in
   let { Plan.update_type = update_type; 
         Plan.update_expr = incr_calc} = dm_stmt 
   in 
   ...
   let incr_result, new_meta = calc_to_k3_expr ~generate_init:true 
                                               meta trig_args_el 
                                               incr_calc 
   in
   let (rhs_outs_el, rhs_ret_ve, incr_expr) = incr_result in
   assert (ListAsSet.seteq (ListAsSet.diff lhs_outs_el trig_args_el)
                           rhs_outs_el);
...
let get_map_from_schema (map_name: string) 
                        (k3_prog_schema: K.map_t list) :
                        K.map_t =
...
let get_map_expr_from_schema (collection_name: string) 
                     (k3_prog_schema: K.map_t list) : 
                     K.expr_t =
   let (mapn, lhs_ins, lhs_outs, map_type) = 
...
let get_collection_status (collection_name: string) 
                          (k3_prog_schema: K.map_t list) : 
                          K.expr_t =
...
let get_map_vars (collection_name: string) 
                 (k3_prog_schema: K.map_t list) : 
                 T.var_t list * T.var_t list =
   let (mapn, lhs_ins, lhs_outs, map_type) = 
      get_map_from_schema (collection_name) (k3_prog_schema) 
   in
      lhs_ins, lhs_outs
...
let string_ends_with (source: string) (test: string): bool = 
   ...
   if source_length < test_length 
   then false
   else 
      let source_last_part = 
         String.sub source 
                    (source_length - test_length) 
                    test_length 
      in
...
let collection_ivc_gc_var stmt_info =
   ...
   let collection_ivc_gc_t  = 
      if free_lhs_outs_el = [] 
      then K.TTuple([domain_type;domain_type])
      else K.Collection(K.Unknown, 
                        K.TTuple( (k3_expr_to_k3_type (free_lhs_outs_el)) @
...
let collection_ivc_gc_generate stmt_info =
   ...
   let is_ivc_result = K.Tuple(rhs_outs_el @ 
                               [ is_ivc_constant; new_value]) 
   in
   let is_normal_result = K.Tuple(rhs_outs_el @
                                  [ is_normal_constant; new_value ]) 
   in
   let is_gc_result = K.Tuple(rhs_outs_el @ 
                              [ is_gc_constant; new_value ]) 
   in
   ...
   let collection_ivc_gc = 
      let inner_loop_body = lambda (rhs_outs_el @ 
                                    [rhs_ret_ve]) ivc_gc_specifier 
      in
      if rhs_outs_el <> [] 
      then K.Map(inner_loop_body, incr_expr)
      else K.Apply(inner_loop_body, incr_expr)
   in
      (collection_ivc_gc)
...
let update_status_statement_generate k3_prog_schema stmt_info =
   ...
   let collection_ivc_gc_var = 
      collection_ivc_gc_var stmt_info 
   in
   ...
   let update_status_lambda = 
      lambda (rhs_outs_el @ [rhs_ret_ve; multi_ret]) 
             update_status_lambda_body 
   in
   if rhs_outs_el = [] 
   then K.Apply(  update_status_lambda,collection_ivc_gc_var)    
   else K.Iterate(update_status_lambda,collection_ivc_gc_var)  
         
let ivc_do (map_name_domain: string) (vars: K.expr_t list) 
           (k3_prog_schema: K.map_t list) : K.expr_t =
   Debug.print "DEBUG-DM-LOG" (fun () -> 
      "ivc_statement_generate: " ^ map_name_domain);
   ...
   let ivc_statement = 
      match (pair_map varIdType_to_k3_expr map_schema) with
          | x, [] -> 
             K.PCValueUpdate(map_expr, vars, [], init_val_from_type map_type) 
          | [], y -> 
             K.PCValueUpdate(map_expr, [], vars, init_val_from_type map_type)
          | x,  y -> 
             failwith "Map with both variables is not supported"
   in
      (* in order to solve the problem when something is 
         initiliazed in ON SYSTEM *)
...
let ivc_statement_generate k3_prog_schema stmt_info =
   ...
   let dummy_statement = get_unit_statement () in
   ...
   let ivc_statement_lambda = 
      lambda (rhs_outs_el @ [ rhs_ret_ve; multi_ret ])
             ivc_statement_lambda_body 
   in
   if rhs_outs_el = [] 
   then K.Apply(  ivc_statement_lambda,collection_ivc_gc_var)    
   else K.Iterate(ivc_statement_lambda,collection_ivc_gc_var)
        
let gc_do (map_name_domain: string) 
          (vars: K.expr_t list)  
          (k3_prog_schema: K.map_t list) : 
          K.expr_t =
   Debug.print "DEBUG-DM-LOG" (fun () -> 
      "gc_statement_generate: " ^ map_name_domain);

   let input_postfix = "_input" in let output_postfix = "_output" in
   (* in_out is true if it's an input, otherwise false*)  
   let in_out = (string_ends_with map_name_domain input_postfix) in
   let postfix = if in_out then input_postfix else output_postfix in
   let map_name = String.sub map_name_domain 0 
                             ((String.length map_name_domain) -
                              (String.length postfix)) 
   in
   let map_schema = get_map_vars (map_name) (k3_prog_schema) in
...
let gc_statement_generate k3_prog_schema stmt_info =
   ...
   let gc_statement_lambda_body = 
      ...
      K.IfThenElse(
         ...
         K.Block([ gc_statement_main; 
                   K.PCElementRemove(lhs_collection, [], lhs_outs_el) ]),
         dummy_statement
      )
   in
   let multi_ret = K.Var("multi_ret",K.TBase(T.TInt)) in
   let gc_statement_lambda = 
      lambda (rhs_outs_el @ [rhs_ret_ve; multi_ret]) 
             gc_statement_lambda_body 
   in
   if rhs_outs_el = [] 
   then K.Apply(  gc_statement_lambda,collection_ivc_gc_var)    
   else K.Iterate(gc_statement_lambda,collection_ivc_gc_var)
...
let update_domain_statement_generate stmt_info = 
   ...
   in
   let update_domain_lambda = 
      lambda (rhs_outs_el @ [rhs_ret_ve; multi_ret]) 
             update_domain_lambda_body 
   in
   if rhs_outs_el = [] 
   then K.Apply( update_domain_lambda, collection_ivc_gc_var )
   else K.Iterate( update_domain_lambda, collection_ivc_gc_var )

(*
let dm_stmt_to_k3_stmt (meta: meta_t) trig_args 
                       (dm_stmt: Plan.stmt_t) 
                       (k3_prog_schema: K.map_t list) : 
   ...
   in
   let collection_ivc_gc = 
      collection_ivc_gc_generate stmt_info 
   in 
   let collection_ivc_gc_var = 
      collection_ivc_gc_var stmt_info 
   in
   ...
   let statement_expr =
      ...
      let lambda_body = 
         ...
         let update_status = 
            ...
            then update_status_statement_generate stmt_info 
                                                  k3_prog_schema 
            else dummy_statement 
         in
         let ivc_statement =
            if Debug.active "DEBUG-DM-IVC" 
            then ivc_statement_generate stmt_info 
                                        k3_prog_schema
            else dummy_statement 
         in
            K.Block([ update_domain_statement; 
                      update_status; 
                      ivc_statement; 
                      dummy_statement ])
      in
...
let dm_trig_to_k3_trig (meta: meta_t) 
                       (m3dm_trig: M3.trigger_t) 
                       (k3_prog_schema: K.map_t list) 
                       (m3_to_k3_trig: K.trigger_t) : 
                       K.trigger_t * meta_t =
   Debug.print "DEBUG-DM-LOG" (fun () -> 
      "dm_trig_to_k3_trig for event: " ^
      (Schema.name_of_event m3dm_trig.M3.event));
   let trig_args = Schema.event_vars m3dm_trig.M3.event in
   let m3_to_k3_stmts = 
       if (Debug.active "DEBUG-DM-WITH-M3") 
       then snd m3_to_k3_trig 
       else [] 
   in
   let k3_trig_stmts, new_meta = 
      List.fold_left (
         fun (old_stms, om) m3dm_stmt -> 
            let k3_stmt, nm = 
               dm_stmt_to_k3_stmt om trig_args m3dm_stmt k3_prog_schema 
            in 
               (old_stms@[k3_stmt], nm) 
      ) ([],([],snd meta))
...
let dm_trig_to_k3_trig (meta: meta_t) 
   ...
   let stmt_infos, new_meta = 
      List.fold_left (
        ...
        !(m3dm_trig.M3.statements) 
   in
   if List.length stmt_infos = 0 
   then ((m3dm_trig.M3.event, m3_to_k3_stmts), meta)
   else    
      let m3_part = K.Block(m3_to_k3_stmts) in
      let update_domain_part =
         K.Comment("Updating domains", 
            K.Block(List.map (update_domain_statement_generate) stmt_infos)
         )
      in
      Debug.print "DEBUG-DM-LOG" (fun () -> "update domain part finished!");
      let update_status_part =
         if Debug.active "DEBUG-DM-STATUS" 
         then K.Comment("Updating status",  
                 K.Block(
                    List.map (update_status_statement_generate k3_prog_schema) 
                             stmt_infos 
                 )
              )
         else get_unit_statement ()
      in
      Debug.print "DEBUG-DM-LOG" (fun () -> "update status part finished!");
      let ivc_part = 
         if Debug.active "DEBUG-DM-IVC" 
         then K.Comment("IVC computation",
                 K.Block(
      ...
      let gc_part = 
         if not (Debug.active "DEBUG-DM-NO-GC") 
         then K.Comment("Garbage Collecting",
                 K.Block(
...
(** Transforms an existing K3 program with its corresponding 
    M3DM program into a K3 program. *)
let m3dm_to_k3 (m3tok3_program : K.prog_t) 
               (m3dm_prog: M3DM.prog_t) : 
               (K.prog_t) =
   let with_m3 = Debug.active "DEBUG-DM-WITH-M3" in
   if with_m3 
   then (); 
      (* begin Debug.activate "DEBUG-DM-IVC"; 
               Debug.activate "K3-NO-OPTIMIZE"
               (* Debug.activate "K3-NO-OPTIMIZE-LIFT-UPDATES" *) 
         end; *)
   let ( k3_database, (old_k3_prog_schema, old_patterns_map), 
         m3tok3_prog_trigs, old_k3_prog_tlqs) = m3tok3_program 
   in
   let k3_prog_tlqs = if (with_m3) 
                      then old_k3_prog_tlqs 
                     else [] 
   in
   let k3_prog_schema = (if (with_m3) 
                         then old_k3_prog_schema 
                         else []) @ 
                        (List.map m3_map_to_k3_map 
                                  !(m3dm_prog.M3DM.maps)) 
   in
         ...
         List.fold_left
            (fun (old_trigs,om) m3dm_trig -> 
               let m3_to_k3_trig =
                  ...
                  List.find (fun (e, sl) -> e = ev) m3tok3_prog_trigs
               in
               let k3_trig,nm = 
                  dm_trig_to_k3_trig om 
                                     m3dm_trig 
                                     k3_prog_schema 
                                     m3_to_k3_trig 
               in
