...
struct
   ...
   let string_of_aggregate = Types.string_of_const
   ...
   (* assumes valuation theta and (Env.make current_outv k)
      are consistent *)
   let concat_keys (current_outv: string list) (desired_outv: string list)
         (theta: Env.t) (extensions: string list) (m: t) =
      let key_refiner k v =
   ...
   (* assumes valuation theta and (Env.make current_outv k)
      are consistent *)
   let project_keys (pat: M3ValuationMap.pattern) (pat_outv: string list)
        (desired_outv: string list) (theta: Env.t)
        (extensions: string list) (m: t) =
      let aux pk kv =
         let aggv = Arithmetic.eval (Arithmetic.ValueRing.mk_prod (
            List.map Arithmetic.mk_const (List.map snd kv)
         )) in
   ...
   let filter (outv: string list) (theta: Env.t) (m: t) : t =
