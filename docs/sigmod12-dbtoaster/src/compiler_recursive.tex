...
~~~   foreach $z,u$: $M_{\Delta_{+S(z,u)} Q}[z,u][]$ $+$= $(y<z)$ ? $(x*u)$ : $0$; \\
~~~   foreach $z,u$: $M_{\Delta_{-S(z,u)} Q}[z,u][]$ $+$= $(y<z)$ ? $(x*u)$ : $0$
\end{tabbing}
...
The workload has to be further decomposed and simplified, otherwise the
...
materialized views to be represented as maps $M$ that store, for tuples $\vec{i}{o}$ of input and output variables, a value (an aggregation result) $M[\vec{i}][{o}]$.
