...
But does this expectation translate into real performance gains? A priori, the cost of the bulk addition operation of materialized views or the costs associated with storing and managing additional auxiliary materialized views (for delta queries) might be more considerable than expected.
...
\comment{
As we see, this generalization allows us to nicely leverage the machinery required to handle some of the other challenges pointed out above, and it makes the approach applicable to general practitioner's languages such as SQL.
}
