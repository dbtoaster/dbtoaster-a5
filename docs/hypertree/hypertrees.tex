...
%\STATE $node\gets$ node representing $\Delta_{R}(J_{1})$
\ELSE
...
%\STATE $node\gets$ a union node with children $\{rest.expr\Join \Delta(J_{1}),\Delta(J_{1})\Join rest.join,J_{1}\Join rest.expr\}$
\ENDIF
...
\bibitem{treewidth}N.Robertson,P.D.Seymour,\emph{Graph minors II,Algorithmic aspects of treewidth}, J.Algorithms 7(1986) 309–322.
\end{thebibliography}
\end{document}
