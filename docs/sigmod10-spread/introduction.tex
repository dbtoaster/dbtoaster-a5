...
aggregate views online.
In that sense it differs from the literature on online aggregation that
is primarily concerned with sampling-based techniques that produce
approximate results
...
main memory. The financial premium of main memory over disks is
...
The Cumulus protoype uses our results to
push OLAP into the cloud.
...
relational data.  Cumulus  keeps the
data warehouse synchronized with the source OLTP databases via an
...
\item
We present Cumulus, our realtime exact online aggregation system.
We describe the Cumulus message passing protocol, which assures
consistency of the maps using few messages, and
