...
for i in 7900000; do
    "$bindir/ssb.exe" triggers $i $ssbdir/out/ssb_tg_"$i".log $ssbdir/results/ssb_tg_"$i".txt $ssbdir/out/ssb_tg_"$i".stats $ssbdir/data a $ssbdir/win/mssql/ssb_triggers.sql > $ssbshelldir/out/ssb_tg_"$i".dump 2>&1
done
