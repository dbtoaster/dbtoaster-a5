...
create stream AtomPositions (
    ...
    y       float,
    z       float
)   FROM FILE '@@DATASET@@/atompositions.csv'
  LINE DELIMITED CSV;
...
)   FROM FILE '@@DATASET@@/atommeta.csv'
  LINE DELIMITED CSV;
...
create table Dihedrals (
    ...
    n           float,
    delta       float
)   FROM FILE '@@DATASET@@/dihedrals.csv'
  LINE DELIMITED CSV;
...
create table ImproperDihedrals (
    ...
    force_const float,
    delta       float
)   FROM FILE '@@DATASET@@/improperdihedrals.csv'
  LINE DELIMITED CSV;
...
create table NonBonded (
    ...
    charge1     float,
    charge2     float
)   FROM FILE '@@DATASET@@/nonbonded.csv'
  LINE DELIMITED CSV;
...
create table ConformationPoints (
  ...
  t             int,
  point_id      int
) FROM FILE '@@DATASET@@/conformationpoints.csv'
  LINE DELIMITED CSV;
...
create table Dimensions (
    ...
    atom_id4    int,
    dim_id      int
) FROM FILE '@@DATASET@@/dimensions.csv'
  LINE DELIMITED CSV;
...
--
select 1 as q3
from Bonds B, AtomPositions P, Angles A, Dihedrals D, AtomMeta M, ImproperDihedrals ID, NonBonded NB;