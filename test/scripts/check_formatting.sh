...
get_max_column() {
    retval=`awk 'BEGIN{max=0; ln=0} {if (length > max) {max=length; ln=NR}} END{print max,ln}' $1`
...
not_within_column_limit() {
    get_max_column $1
    if [ $width -gt 80 ]
    #   then echo `awk '{ printf "Line: %-5s  Width: %-4s in %s\n", $line, $width, $1}' $1` 
...
echo "********** TAB CHECK **********"
find src test lib -type f \( -name '*.ml' -o -name '*.mly' -o -name '*.cpp' -o -name '*.hpp' \) \
     -exec grep -l $'\t' {} ";"
