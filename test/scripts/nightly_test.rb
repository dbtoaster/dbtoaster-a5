...
$log_targets = [
  {
    :type     => :email,
    :address  => "okennedy@buffalo.edu",
    :detail   => :status
  },
  {
    :type     => :email,
    :address  => "milos.nikolic@epfl.ch",
    :detail   => :full
  },    
  {
    :type     => :email,
    :address  => "daniel.lupei@epfl.ch",
    :detail   => :full
  },
  {
    :type     => :growl,
...
#loop do
  ...
  $tests = [
    ...
    f.readlines.map { |l| l.chomp }.delete_if { |l| l == "" || l.start_with?("#")}.
      map do |l| 
  ...
  msg "Compiling"
  system('make clean; make COMPILE_SCALA=1');
  msg "Running Tests"
  $tests.each do |t|
