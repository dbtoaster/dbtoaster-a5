...
SELECT s.s_suppkey, 
       quote_literal(s.s_name), 
       quote_literal(s.s_address), 
       quote_literal(s.s_phone), 
       r1.total_revenue,
       COUNT(*)
FROM supplier s, 
...
WHERE s.s_suppkey = r1.supplier_no
...
GROUP BY s.s_suppkey, s.s_name, s.s_address, s.s_phone, r1.total_revenue;
