...
<p>
   Starting with OS X Mavericks, g++ is just a symlink to clang and the default library is libc++ instead of libstdc++.
   DBToaster is currently not compatible with libc++ and depends on boost.
   Thus, DBToaster requires the boost library to be built with libstdc++.
</p>

<p>
   <i>Note:</i> We are currently working on a new version of the DBToaster library that does not depend on boost and is fully compatible with libc++, so the following procedure will not be necessary in the future.
</p>

<p>
   First, gcc has to be installed. This can be done with the following commands:
</p>
...
<p>
   Now, gcc can be used to build boost.
   Download boost from <a href="http://www.boost.org/users/download/">here</a>, unpack the archive and open <tt>tools/build/v2/user-config.jam</tt>.
...
<div class="codeblock">using darwin : 4.9 : g++-4.9 ;</div>

<p>
   Now, the compilation can be launched with:
</p>

<div class="codeblock">
$> ./bootstrap.sh --with-libraries=program_options,serialization,system,filesystem,chrono,thread --with-toolset=darwin --prefix=/usr/local
$> ./b2
</div>

<p>
   After the compilation, copy the <tt>.a</tt> files from <tt>stage/lib</tt> in the boost directory to <tt>lib/dbt_c++</tt> in the DBToaster directory and the <tt>.dylib</tt> to the directory where you are storing the C++ binaries generated by DBToaster.
