...
start with a query $q$, and produces a pair $\tuple{m,q'}$ corresponding to the
materialization of $q$, and the delta query responsible for maintaining the
materialization respectively. The delta query $q'$ can be thought of as a
parameterized SQL query, with parameters corresponding to attributes of an
update to a delta table. On an update $u$, IVM performs the work: $m \mbox{
...
property of delta queries: \textit{a delta query is often simpler than its
parent query}. In particular \textit{k}-th order delta query, $q^k$ has fewer
input relations than a \textit{k-1} order query $q^{k-1}$, but additional
...
similar to TPH-C Query 3. 
The map produced as the view for $q$ is $m[ordkey,sprior]$. We can answer $q$
from the key-value entries in $m$. The first-order delta query $q'$ includes a
...
\noindent We omit \texttt{for} loops in pseudocode, they are implicit based
on function arguments (see line 2 for the above statement). Note
$ordkey$,$sprior$ are not arguments, indicating a loop.
...
query does not depend on the database (there are no relations, only parameters).
Our algorithm considers all permutations of updates, for example deltas of $q$
for insertions and deletions to {\tt order} or {\tt lineitem}. Due to limited
space, we only show the transition program for insertions, with $m',m''$ renamed
$m\_c,m\_cl$:
...
trees, and no nested loops, only single-level loops that perform probing as
in hash joins.
\comment{
...
the design of main-memory query processors and touch on this below.
...
belief propagation and message passing algorithms, Viterbi's algorithm, and
FFTs. With this analogy, we can leverage other techniques from this field, for
