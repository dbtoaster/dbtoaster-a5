...
relations, only parameters). Our compilation algorithm considers all possible update orderings, for example deltas of $q$ for insertions and deletions to {\tt order} and then {\tt lineitem}.  We express the path taken as part of the map name: $m'$ and $m''$ are $m\_c$ and $m\_cl$ respectively.  Due to limited space, we only show the transition program for insertions.
