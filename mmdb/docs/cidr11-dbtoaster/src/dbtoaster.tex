...
denote this by a two-level map $m[][orderkey, spriority]$. Since we materialize
...
materialization as $m'[ck][orderkey,spriority]$, where $m'$ is a map
...
materializes each delta query encountered. Our algorithm detects and reuses
duplicate maps based on equivalent delta queries. The full transition program
is:
