...
perform probing as found in hash joins. The transition program is also
exploiting distributivity to push down aggregates through joins, yielding small
...
\todo{[PODS REF]}, where queries are represented as polynomials, admit
transformations compactly represented by an algebraic structure, namely rings, and demonstrates
...
extremely important. Our framework has many applications, even when considering
the simple class of equi-join with sum aggregations as shown above. This class
of queries, when viewed through the lens of a semiring or ring, can be
considered an application of the generalized distributive law (GDL) \todo{[REF
Aji, McEliece]} to query processing. GDL enables a large class of fast
algorithms for a variety of applications including belief propagation and other
message passing algorithms, Viterbi's algorithm, FFTs and so forth. Drawing this
analogy to GDL, we can begin to consider extremely efficient large-scale
evaluation of these applications using query processing techniques based on
DBToaster. \todo{Eddies?}
