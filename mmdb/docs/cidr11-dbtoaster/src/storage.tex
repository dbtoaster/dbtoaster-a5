...
{\bf Datastructures}.
%
As seen in Section~\ref{sec:compilation},
DBToaster uses multi-key (i.e., multi-dimensional) maps to represent materialized views.
The generated view maintenance code performs lookups of slices of the maps,
using partial keys, fixing some dimensions and iterating over the others.
Recall the right-hand side of line 2 in the code listing in Section~\ref{sec:compilation}.
This is a partial lookup on map $m_c$ with only {\tt ck} a trigger
argument.
%
...
A transition program statement reads one map and writes another,
prescribing a dependency between
pairs of locations in the right-hand side (reading) and the left-hand side
(writing) map.
...
and any horizontal partitioning of the pair of a reading and a writing
...
solving a straightforward optimization problem.
