...
\nop{
Above, we have implicitly assumed that maps $m$ are defined everywhere,
or at least on all the argument tuples that we will encounter while
running an M3 program. We have assumed that the program starts on an
empty database (no tuples have been inserted yet) and all maps have value
0 everywhere, which is correct because all maps are defined to represent
$\AggSum$ terms, which are 0 on an empty database.

Unfortunately, it is not a realistic assumption that map domains are known
in advance: We cannot know what the tuples inserted later will contain,
and incrementing maps for all values of an absurdly large domain
is infeasible too.
...
\begin{figure}
\begin{center}
\includegraphics[width=3in]{images/q12_graph.pdf}
