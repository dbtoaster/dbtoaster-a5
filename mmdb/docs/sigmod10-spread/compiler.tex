...
\begin{example}\em
The query of Example~\ref{ex:nation} translates to
$\AggSum(1, \mbox{Customer}(c,n_c) \land \mbox{Supplier}(s, n_s) \land
n_c=n_s)$ in the calculus, with bound variable $s$.
\punto
\end{example}
...
\begin{algorithm} { }~{ }
\begin{verbatim}
function Compile(t: term, bound_vars: var list)
returns an M3 program
begin
   for each relation R in the schema do
      args = turn schema into list of new argument
             variable names;
      tR = Simplify((Delta_{R(args)} t), bound_vars);
      tRs = RecMonomials(tR);
      for each t_i in tRs do
         (t'_i, Theta_i) =
             ExtractAggregates(t', bound_vars);
...
\begin{todo}
Explais RecMonomials.
Explain how ``for x m[x] += if(x=b) ...''
is turned into ``x[b] += ...''.
\end{todo}
...
\begin{example}\em
We continue Example~\ref{ex:selfjoin}.
