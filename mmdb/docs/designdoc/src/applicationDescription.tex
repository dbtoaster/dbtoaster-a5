...
class QueryTupleQueue
{
	...
	MemoryManager;
	SchedulingQueue*;
	map<sourceID, listTupleQueue>  sourceAccess;
	mpa<queryID, listTupleQueue>   queryAccess;
	
}
\end{verbatim}

\noindent Function {\tt addDatum(readerID, structuredTuple)} is called by the Data Dispatch to add a new data item. Function {\tt getDatum(queryID)} is required to extract data items available for the query to be executed. Functions {\tt addQuery(queryID, list<int> readersID)} and {\tt removeQuery(queryID)} are needed by Query Admin to insert or remove a new query.

To enable this functionality we need to consider security and efficiency. 

\subparagraph{Security:} By security we mean that no data tuple can be lost or received out of order by the query. This can be enabled by a simple procedure of locking each queue when data is inserted or removed from each queue.

\subparagraph{Efficiency:} The main inefficiency comes form the fact that data from a data source is needed by several queries. If we adopt a simple model where each query has its own set of queue from each of the required sources, we have inefficiency of coping data to each such queue. The benefit of such model is simplicity of implementation and use. Another model is to have only one queue for each data source. In this case each queue will need to keep track of which query requires what item from this queue as well as making sure that the items not required by any query are removed. To support both approaches we can hide all this details into the implementation of the queues. By requiring the following specification:

\begin{verbatim}
class TupleQueue
{
	TupleQue*         join(queryID);
