...
For instance, if we are using drop strategy and have historical relation \emph{histView} and simulated relation \emph{simView}. 
\begin{program}
    \begin{verbatim}   
    while (input = getNextTuple())
    {
        storeTuple(input);
        if (heartbeat)
        {
    ...
    \end{verbatim}
\caption{Exchange Matching for drop.}
\end{program}

%\emph{add a complete list of possible matchings and show how they will be implemented}.
