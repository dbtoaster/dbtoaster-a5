...
\\
\label{eq:pred4}
\sigma_{g_1=B} (Q) &:=& \pi_{g_1\rightarrow B}(bind_{g_1\rightarrow B}(Q))
\\
\label{eq:pred5}
\sigma_{g_1=B \wedge f[\vec{A}, g_1 \dots g_k] \theta 0}(Q) &:=&
\sigma_{f[\vec{A}, g_1 \dots g_k] \theta 0}(\pi_{g_1 \rightarrow B}(bind_{g_1 \rightarrow B}(Q)))
\\
...
\hline \ref{eq:sum4} & missing & (simplify, similar to rule \ref{eq:sum4}) \\
\hline \ref{eq:sum5}$\sim$\ref{eq:sum7} & complete & simplify \\
\hline \ref{eq:min1} & missing & (check later)\\
\hline \ref{eq:min2}& complete & simplify \\
\hline \ref{eq:min3}$\sim$\ref{eq:min4} & missing & (simplify, similar to rule \ref{eq:sum4}, needs to add `Max)\\
\hline \ref{eq:min5} & complete & simplify \\
\hline \ref{eq:min6} & missing & (simplify, similar to rule \ref{eq:sum4}) \\
\hline \ref{eq:min7} & missing & (simplify, might need state identifier to hold value of min f and min g) \\
...
\hline \ref{eq:pred3} & missing & (extract\_plan\_bindings)\\
\hline \ref{eq:rs1} & complete & simplify \\
\hline \ref{eq:md1} & complete & compute\_new\_plan\\
\hline \ref{eq:md2}$\sim$\ref{eq:md3} & missing & (need `Delete, `IncrDiffPlan. similar to rule \ref{eq:md1})\\
...
\hline \ref{eq:na1}$\sim$\ref{eq:na5} & missing & (not now) \\
\hline
