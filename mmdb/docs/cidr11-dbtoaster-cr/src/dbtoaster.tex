...
Modern relational engines, and their operator and Volcano-style iterator
abstractions, lack a clear, flexible representation of many aspects of query
...
The primary features of our FP language are its use of
nested collections (such as sets, bags and lists) during query processing, its
ability to perform structural recursion (SR)~\cite{buneman-kleisli:95}
optimizations on such nested collections, and its support for long-lived
...
transform the granularity of data processing. Consider a join tree
$(R \bowtie S) \bowtie T$, and for simplicity assume there are no indexes and
that the joins are implemented in nested loops form. The functional
representation is:

\begin{verbatim}
flatten(map(f <a,b>. map(f c.<a,b,c>, T),
  flatten(map(f x. map(f y.<x,y>, S), R))))
\end{verbatim}

The classical implementation of joins as binary operations forces the
materialization of the intermediate result $R \bowtie S$. There are numerous
scenarios where such materialization is undesirable, and has led to the
...
\begin{verbatim}
flatten(flatten(
  map(f x. map(f y. map(f z.<x,y,z>, T), S), R)))
\end{verbatim}

This is essentially a three-way nested loops join $\bowtie_3(R,S,T)$ with no
intermediate materialization of a two-way join. When combined with a
non-blocking (i.e. lazy) functional map operation, and data structures such as
tree-based indexes and hash tables, we can apply these transformations to create
and optimize over a general space of join plans, which critically vary in both
the arity (and thus materialization), pipelining and ordering of join
operations. This fine grained control over the granularity of an operator, as
provided by alternative techniques such as operator fusion and superboxes as
found in stream engines, can be jointly manipulated with scheduling strategies,
provided the scheduler is implemented in the same framework.

Notice the reordering of {\tt flatten} operations indicating an internal
result form consisting of nested collections.
}
