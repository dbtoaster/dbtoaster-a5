...
Surprisingly, despite more than three decades of research into incremental view
maintenance (IVM)
techniques~\cite{roussopoulos-tods:91,griffin-sigmod:95,zhou-icde:07,zhou-vldb:07},
this goal has not been realised, and one of our key contributions in handling
...
(essentially higher-order delta queries) to maintain these auxiliary views.
...
evaluates a delta query entirely from scratch on every update to any relation
in $q$, using standard query processing techniques. DBToaster exploits that a
delta query $q'$ can be incrementally computed using the same principles as for
...
to a relation in $q$. We call this one step of \textit{delta (query)
...
\textit{recursively}, transforming queries to \textit{higher-order deltas}.
...
different to $u$ above. We refer to $q'$ and $q''$ as first- and second-order
delta queries respectively. We can again recur with each $q''$, materializing as
$m''$, maintaining with $q'''$, a third-order delta, and so forth.

\comment{
DBToaster performs recursive delta compilation to compute and
materialize higher-order delta queries. Let us consider higher-order deltas in
...
parameterized SQL queries, with parameter values taken from updates. We can see
this when applying one step of delta compilation on the following query $q$ over
a schema {\tt R(a int, b int), S(b int, c int)}:

$q = \mbox{\tt select sum(a*c) from R natural join S}$

\vspace{1mm}
\noindent For an update $u = \tuple{a,b}$ on relation $R$, the delta for $q$ is:
...
\vspace{-2mm}
\noindent The {\tt values} clause is PostgreSQL syntax for a singleton relation.
...
The second property is that delta queries have a normalized form, where we
transform queries to be aggregates over unions of joins (all selections,
projections appear intermixed with joins). This is written in relational algebra
as: $\sum [ Q_1 \cup Q_2 \cup \ldots \cup Q_n ]$, where each $Q_i$ is a
relational algebra expression that does not contain a union, for example
$Q_1 = \sigma_{c}(\pi_a(R_1) \bowtie_{\theta} R_2)$. This is similar to a
disjunctive normal form in boolean logic, and a sum-product form when
considering databases queries in terms of rings~\cite{koch-pods:10}.
We refer to the subqueries $Q_i$ in such a normal form as \textit{conjuncts}.

This normal form is important when considering termination of recursive
compilation. DBToaster terminates given one critical property of delta queries:
\textit{a delta query is structurally simpler than its parent query}. A natural
measure of delta query complexity is that of \textit{degree}, which we define as
the maximal number of relations in any conjunct $Q_i$ in a query. For example
$degree(\sigma_c(\pi_a(R_1) \bowtie_{\theta} R_2)) = 2$.
DBToaster guarantees that a \textit{k}-th order delta query $q^{(k)}$ has lower
degree than a \textit{k-1}-th order query $q^{(k-1)}$.
Recursive compilation terminates when all conjuncts have degree 0.

Consider the delta query $q_R$ above. This query is simpler than $q$ since it
does not contain the relation $R$. We can further illustrate the point by
looking at a recursive compilation step on $q_R$. The second compilation step
materializes $q_R$ as:
...
with the following delta query on update $v = \tuple{c,d}$ to relation $S$:

\vspace{1mm}
$q_{RS} = \Delta_v(q_R) = \mbox{\tt select @c from values(@c,@d)}$

\vspace{1mm}
\noindent The delta query $q_{RS}$ above has degree 0 since its conjuncts
...
In this compilation trace, we have not discussed the maintenance code for views
$m$, $m_R$, and $m_{RS}$ to allow the reader to focus on the core recursive
...
DBToaster materializes higher-order deltas as agile views for high-frequency
...
\tinysection{Towards a main-memory query engine}
...
structural recursion (SR)~\cite{buneman-kleisli:95} optimizations on such nested
collections, and its support for long-lived persistent collections.
Structural recursion optimizations enable the elimination of temporaries and
intermediates when manipulating collections, and when combined with primitive
operations on functions, such as function composition, yields the ability to
transform the granularity of data processing. Consider a join-aggregate query
$\sum_{a*f}((R \bowtie_{\theta} S) \bowtie_{\theta} T)$ with schema
$R(a,b)$, $S(c,d)$, $T(e,f)$. That is the natural joins result in Cartesian
products. The functional representation is:
...
\todo{Description of this program.}
...
development of multiway join algorithms such as XJoin and MJoin \todo{[REFS]}.
With a few simple transformation steps, we can rewrite the above program to
avoid this intermediate materialization as:
