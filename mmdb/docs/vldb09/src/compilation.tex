...
either sum, max or min. count is sum$_1$. (Note this is incomplete right now,
I will finish once the rewrite rules are settled.)}
\label{fig:maprules}
\end{figure*}
...
(\algsum_{f(\cdot)}(R))[\vec{b}]
& \rightarrow &
\lambda(a,b).a*\algsum_{f(\cdot)}(R)[b]
\label{cr1}
\\
\algsum_{f(\vec{A})}(\rho_{\vec{A}}(R)) *
...
\label{cr2}
\\
(\algsum_{f(\vec{A})}(\rho_{\vec{A}}(R)))[\vec{a}] \wedge 
(\algsum_{g(\vec{B})}(\rho_{\vec{B}}(S)))[\vec{b}]
& & \nonumber
\end{eqnarray}
\end{center}
\caption{Code generation rules.}
