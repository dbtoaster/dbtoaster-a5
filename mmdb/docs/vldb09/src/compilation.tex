...
\big( \mbox{if ($(g \;\theta\; 0) = (g + \Delta_{+R(\vec{r})} g \;\theta\; 0))$ then $0$}
\\
      \mbox{else if $(g + \Delta_{+R(\vec{r})} g \;\theta\; 0)$ then
          $\Delta_{+R(\vec{r})} \algsumf_A(Q)$} \\
      \mbox{else $- \Delta_{+R(\vec{r})} \algsumf_A(Q)$} \big)
\end{multline*}
\end{algorithm}
%\caption{Foo.}
%\end{figure*}
...
that is, $f$ after inserting $\vec{r}$ into $R$ is
$f + \Delta_{+R(\vec{r})} f$.
%\[
...
\\
&=& k * v \\
&-& \mbox{if $((p - p_2 > 0) = (p-p_2+p-p_2 > 0)$} \\
&& ~~~ \mbox{then $0$
          else if $(p-p_2+p-p_2 > 0)$ then $v$
          else -$v$}
\end{eqnarray*}
...
\\
&& ~~~
      \mbox{sgn}(m[p_2] + \Delta m[p_2]) \\
&& ~~~~~~ * \;  \mbox{$\underbrace{\algsumf_{P_2 * V_2}(\sigma_{P_2=p_2}(\rho_{P_2,V_2}(B)))[p_2]}_{s[p_2]}$ else 0} \big)
\\
&+&
