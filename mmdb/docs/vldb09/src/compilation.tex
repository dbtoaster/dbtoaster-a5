...
\end{multline*}
%
canonically translates to the map algebra as
%
\begin{multline*}
\algsumf_{P_2 * V_2}(
...
Let us now compute the on insert trigger for the query.
...
&=&
\algsumf_{p_2 * v_2}(
\sigma_{k * m_0[] > m_1[p_2]}(
(\rho_{P_2, V_2}(\{\tuple{p_2,v_2}\})))[]
\\
&=&
\mbox{if $(k * m_0[] > m_1[p_2])$ then $p_2 * v_2$ else $0$}
\quad (\mbox{on insert into B values ($p_2, v_2$)})
\end{eqnarray*}
where $m_0[]$ and $m_1[]$ refer to versions of the maps to which the
insertion has already been applied.

The increments for $m_0[]$ and $m_1[P_2]$ are analogously
\begin{eqnarray*}
\Delta m_0[] &:=&
\algsumf_{V_0}(\rho_{P_0, V_0}(\{\tuple{p_0, v_0}\}))[] = v_0
\\
&& (\mbox{on insert into $B$ values ($p_0, v_0$)})
\\[1.5ex]
\Delta m_1[p_2] &:=&
\algsumf_{V_1}(\sigma_{P_1 > p_2}(\rho_{P_1, V_1}\{\tuple{p_1, v_1}\})))[p_2]
\\
&=& \mbox{if $(p_1 > p_2)$ then $v_1$ else $0$}
\\
&& (\mbox{for each $p_2$, on insert into $B$ values ($p_1, v_1$)})
\end{eqnarray*}
...
expressions $q[\vec{x}]$ into C code that incrementally maintains $q[\vec{x}]$.
...
\begin{definition}\em
A singleton relation is a relation with one tuple, $\{\tuple{a}\}$.
...
the maps can be reformulated as a simple arithmetic expression over its arguments.
