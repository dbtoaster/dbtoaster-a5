...
Cumulus is built around a novel hybrid consistency execution framework.  Like most cloud data management systems, Cumulus achieve low latencies by retreating to the realm of eventual consistency.  However, unlike these systems, Cumulus periodically generates a consistent snapshot of its results as a side effect of it's normal data processing operations.  The result is a system that can be employed side-by-side by applications that require low latency operating side by side with applications that require consistency.
...
a natural side effect of Cumulus' normal data processing is a perio

 retreats to the realm of eventual consistency in order to maintain low latencies.  

breaking view maintenance down to it's most basic representation, Cumulus is able to generate a low-latency, 

is based around a hybrid consistency model.  

 Using a novel new hybrid consistency evaluation scheme, Cumulus is able 
