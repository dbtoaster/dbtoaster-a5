...
\parsection{Input/Output Variables}
...
Variable unification has an impact on an expression, since input variables can be unified with output variables.  The input variables can not be removed, but if unified with an output variable, they become output variables themselves.  

Note however, that this does not apply to the LHS of an update expression.  An input loop variable in an update expression can not be unified away \todo{is this correct?}
