...
class StreamRunner
  ...
  def run_hadoop_stream(input, out, mapper, reducer, reducers, extra, 
    ...
    jruby_path = File.join(ENV['PATH'].split(':').select { |p| File.file?(File.join(p, 'jruby')) }[0], "jruby").to_s + " -J-cp " + ENV['JRUBY_CLASSPATH']

    if reducer.nil?
      cmd = "hadoop jar #{HADOOP_STREAMING} " + #ENV['JRUBY_JARS'] + " " +
        "#{input} " +
	"-output NONE " +
	"-mapper \"#{jruby_path} #{mapper}\"" +
