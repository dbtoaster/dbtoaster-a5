    ...
    for (( j=0; j<$old_num_nodes; j++ )); do   
      for (( k=0; k<$num_nodes; k++ )); do
        echo "Repartitioning $j => $k"
          ./bootstrap.rb -l $j -s $k -r $rspec_file -d $input_dir -o $output_dir
      done
    done

# Per-map repartitioning
#     ./bootstrap.rb -l $j -r $rspec_file -d $input_dir -o $output_dir
#  done

# Per-dataset repartitioning
#  ./bootstrap.rb -r $rspec_file -d $input_dir -o $output_dir
