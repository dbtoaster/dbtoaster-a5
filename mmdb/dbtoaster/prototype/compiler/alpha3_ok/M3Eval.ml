...
struct
   ...
   let aggregate (l: (ListMap.key * const_t) list) : 'a t =
      let aux m (k,v) = 
         let v2 = if(ListMap.mem k m) then ListMap.find k m
                  else CInt(0) in
         ListMap.add k (M3.c_sum v v2) m
      in 
...
type slice_t = const_t ListMap.t
type map_t   = slice_t ListMap.t (* nested map *)
type db_t    = map_t StringMap.t

let slice_to_string slice = ValuationMap.to_string M3.string_of_const slice 
...
*)
   ...
   in
   (* check a condition *)
   let int_op op x y = if (op x y) then CBool(1) else CBool(0)
   in
   match incr_calc with
    ...
    | Add (c1, c2)        -> do_op ( M3.c_sum )         c1 c2 theta db
    | Mult(c1, c2)        -> do_op ( M3.c_prod )         c1 c2 theta db
    | Lt  (c1, c2)        -> do_op (int_op (< )) c1 c2 theta db
    | Leq (c1, c2)        -> do_op (int_op (<=)) c1 c2 theta db
    | Eq  (c1, c2)        -> do_op (int_op (= )) c1 c2 theta db
    | And (c1, c2)        -> do_op (fun condA condB -> if (condA<>CBool(0)) && (condB<>CBool(0)) then CBool(1) else CBool(0)) 
                                   c2 c1 theta db
    | IfThenElse0(c1, c2) -> do_op (fun v cond -> if (cond<>CBool(0)) then v else CInt(0))
                                   c2 c1 theta db
    | Null(outv)          -> (outv, ListMap.empty, db)
    | Const(c)            -> ([], ListSMap.from_list [([], c)], db)
...
*)
      ...
      in
      if (ListMap.mem k a) then (M3.c_sum (ListMap.find k a) v2)
      else
(*
         failwith "ARGH, initial value computation did not find value"
*)
         (M3.c_sum v2 (if (ListMap.mem k a) then (ListMap.find k a) else CInt(0)))
   in
   let new_slice = ListSMap.mergei (fun k v -> v) f2 (fun k v1 v2 -> (M3.c_sum v1 v2))
                                   current delta
   in
   (new_slice, db1)
