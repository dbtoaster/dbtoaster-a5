...
struct
   ...
   let wrap_float v = 
      IP.Parens(("K3Value.Float(",")"), v)
   
   let wrap_map v = 
      IP.Parens(("K3Value.SingleMap(",")"), v)
      ...
      (key_tuple ~init_id:init_id ~tuplizer:(fun klist_w_stuff -> 
            let klist = List.map fst klist_w_stuff in
               ...
               if (List.length kt) = 1 then "["^(List.hd klist)^"]" else
                    (Util.string_of_list "::" 
                                         (List.rev (List.tl (List.rev klist)))
                    )^"::["^(List.hd (List.rev klist))^"]"
         ) kt)
   ...
   let recursive_inline (wmap:code_t): code_t = 
      let rec rec_push (recursing:bool) ((map,mapt):code_t): code_t option = (
         ...
         | Collection(DBEntry,kt,vt) ->
            let (imap,imapt) = (match 
                     (rec_push true (IP.Leaf("inner_map"),vt)) with
         ...
         match (rec_push false wmap) with 
         ...
         (apply_one "MC.from_list"
            (apply_many "List.map" [
               IP.Leaf("(fun ("^(key_tuple (kt@[vt]))^") -> "^
                        (key_tuple ~tuplizer:(Util.list_to_string fst) 
                                   (kt@[vt]))^")");
               map
            ]))
      | Collection(DBEntry,kt,vt) -> 
               ...
               apply_many "MC.mapi" [
                  (wrap_function "wrapped_key map_value"
                                 (Some((fun x -> IP.Lines[
                                    ...
                                    (mk_let ("("^(key_tuple (k@[t]))^")")
                                            x);
                                    IP.Leaf(
                                       "(("^(wrap_key_tuple k)^"),"^
                                       (key_tuple ~init_id:(List.length k)
                                                  [t]
                                       )^")"
                                    )
                                 ])))
                                 ("("^(key_tuple k)^","^
                                  (match t with 
                                    | Collection(Inline,k,t) -> "map_value"
                                    | Collection(_,_,_)
                                    | _ -> "map_value"
                                  )^
                                  ")") 
            ...
            (((*wrap_map*) (apply_many "MC.fold" [
               (wrap_function ("enc_key value accum")
                              (Some(fun x -> 
                                 IP.Lines[
                                    (mk_let ("("^(key_tuple k)^")")
                                            (unwrap_key k "enc_key"));
                                    (mk_let (key_tuple ~init_id:(List.length k)
                                                       [t])
                                            (IP.Leaf("value")));
                                    ((gb_aux k t) x)
                                 ]))
   ...
   * for these methods *)
   (* map, key -> bool/float *)
   let exists ?(expr = None) (wmap:code_t) (key:code_t list): 
              code_t =
      let (map,mapt) = (unwrapped_map wmap) in
      match mapt with 
      |  Collection(Inline,kt,vt) ->
