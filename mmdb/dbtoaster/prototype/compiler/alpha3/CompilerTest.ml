...
(Compiler.compile_delta_for_rel "R" [("A", TInt); ("B", TInt)] "m" [] [] pos m) =
...
Compiler.compile sch "m" [("A", TInt)] [] pos m =
...
Compiler.compile sch "m" [("D", TInt)] [] pos
...
Compiler.compile sch "m" [("B", TInt); ("C", TInt)] [] pos
...
Compiler.compile [("R", [("A", TInt); ("B", TInt)])] "m" [] [] pos
(make_term(
RVal(AggSum(RVal(Const(Int 1)),
