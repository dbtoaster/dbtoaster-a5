...
struct dispatch_vwap_tuple_handle
{
     ...
     int operator()(boost::any& inTuple) {
         DBToasterTuple tuple = adaptor(rTuple, inTuple);
...
{
    sources.addStream<DBToaster::DemoDatasets::VwapTuple>(&VwapBids, streamVwapBidsId);
...
//        router.dispatch(dbtInput);
    ...
    void recursive_stepn()
    {
        ...
        {
            sources.read(&DebuggerHandler::recursive_stepn);
        }
    }
...
int main(int argc, char **argv) 
{
    ...
    int port = (70457>>3);
    shared_ptr<DebuggerHandler> handler(new DebuggerHandler(sources, router));
