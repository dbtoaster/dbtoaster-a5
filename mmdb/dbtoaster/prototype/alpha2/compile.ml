	...
	match q with
		...
		| `Union ch ->
			let chq = List.map simplify_plan_constants ch in
				...
				else `Union (List.filter (fun x -> x <> `EmptySet) chq)
...
let rec substitute_code_vars assignments c_expr =
	...
	match c_expr with
		...
		| `AssignMap(mk, c) -> `AssignMap(mk, sa c)
		| `Declare d ->
			begin
				match d with
					| `Variable (n,f) ->
			...
			let (new_decls, substituted_code) =
				let var_assignments =
					List.filter
						(fun c ->
							match c with
								| `Assign(x, `CTerm(`Variable(y))) -> true
								| _ -> false)
								...
								match (m_expr, pred_cterm) with
									| (`Init x, `MapAccess(mf)) ->
										let map_contains_code = `CTerm(`MapContains(mf)) in
											`IfNoElse(
												`BCTerm(`EQ(map_contains_code, `CTerm(`Int 0))),
													`Block(
														[assign_var_code;
