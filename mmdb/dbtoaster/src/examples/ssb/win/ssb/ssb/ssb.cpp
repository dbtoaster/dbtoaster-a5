...
private:
	...
	cliext::pair<bool, T> read_tuple()
	{
		...
		buffer_mem_usage += r.getSize();

		//Console::WriteLine("Read: {0}", r.as_string());
		if ( (line % 50000) == 0 ) {
			Console::WriteLine("Buffer size: {0} {1} {2}",
				line, buffer.size() * sizeof(Lineitem), buffer_mem_usage);
		}
...
public:
	...
	void preload_streams(String^ dir, String^ fileset)
	{
		...
		nation_stream^ nt_s =
			gcnew nation_stream(nt_file, nt_parser, 4, 100, 512);
	...
	stream^ s, StreamWriter^ log, StreamWriter^ results, StreamWriter^ stats)
{
	...
	SqlParameter^ ocmParam = gcnew SqlParameter("@comment", SqlDbType::VarChar, 79);
	...
	array<SqlParameter^>^ orderParams = {
		...
		oodParam, oopParam, oclParam, ospParam, ocmParam };
	...
	while ( s->stream_has_inputs() )
	{
		...
		case 1:
			...
			insertOrderCmd->Parameters[paramCount]->Value = ord->comment; ++paramCount;
	...
	triggerSw->Stop();
	Console::WriteLine("{0} s for 'SSB triggers'",
		...
		QueryResults% running_results, StreamWriter^ results)
	{
		...
		if ( c_nt_found != csnt.end() )
		{
			...
			if ( !(sk_found == sup.end() || pk_found != parts.end()) )
			{
				...
				QueryResults::iterator dyr_found = running_results.find(d_yr);
				if ( dyr_found != running_results.end() )
				{
					if ( dyr_found->second->find(c_nation) != dyr_found->second->end() )
						running_results[d_yr][c_nation] += (lo_rev - lo_supc);
					else
						running_results[d_yr][c_nation] = lo_rev - lo_supc;
				}
				else {
					running_results[d_yr] = cliext::hash_map<String^, Double>();
					running_results[d_yr][c_nation] = lo_rev - lo_supc;
				}
...
public:
		...
		StreamWriter^ log, StreamWriter^ results, StreamWriter^ stats)
	{
		...
		Stopwatch^ tup50kSw = Stopwatch::StartNew();
		...
		while  ( s->stream_has_inputs() )
		{
			...
			if ( (tupleCounter % 50000) == 0 ) {
				tup50kSw->Stop();
				Console::WriteLine("Processed {0} tuples.", tupleCounter);
				Console::WriteLine("Exec time {0}", (tup50kSw->Elapsed.TotalSeconds / 50000.0));
...
stream^ load_streams(QueryProcessor^ qp, String^ dir, String^ fileset)
{
	...
	lineitem_stream^ li_s =
		gcnew lineitem_stream(li_file, li_parser, 16, 1000000, 512);

	//order_stream^ ord_s =
	//	gcnew order_stream(ord_file, ord_parser, 9, 170000, 512);
	...
	cliext::vector<stream^> inputs(1);
	inputs[0] = li_s;
	//inputs[1] = ord_s;
	//inputs[2] = pt_s; inputs[3] = cs_s; inputs[4] = sp_s;
