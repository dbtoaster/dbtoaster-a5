...
public class DebugView extends ViewPart {
	...
	public void createPartControl(Composite parent) {
		...
		insertButton.setLayoutData(new GridData(SWT.CENTER, SWT.CENTER, true, false));
		
		// Delete button
		Button deleteButton = new Button(applyEventComp, SWT.PUSH);
		deleteButton.setText("Delete");
		deleteButton.setLayoutData(new GridData(SWT.CENTER, SWT.CENTER, true, false));
	...
	class Relation {
		...
		public Relation() {}
		public Relation(String ns) { nameAndSchema = ns; }
		...
		public String getNameAndSchema() { return nameAndSchema; }
		...
		public void setNameAndSchema(String s) { nameAndSchema = s; }
		public void setValues(LinkedList<String> v) { values = v; }
	...
	class RelationModifier implements ICellModifier {
		...
		public RelationModifier(Viewer v) { viewer = v; }
	...
	class DataStructureRangeException extends Exception {
		...
		public DataStructureRangeException(String msg) { super(msg); }
	};
	...
	class DataStructureRange {
		...
		public Boolean getIsWholeRange() { return wholeRange; }

		public Boolean getIsSingleValue() { return singleValue; }
		...
		public Integer getValue() { return singleValue? keyValue : null; }

		public void setValue(Integer kv) {
			if ( singleValue ) keyValue = kv;
		}
		
		public DataStructureRange getParent() { return parent; }
		...
		{
			if ( singleValue )
				throw new DataStructureRangeException("Values cannot have children.");
			...
			for (DataStructureRange dsr : children) {
				...
				else if (dsr.singleValue &&
						clb <= dsr.keyValue && dsr.keyValue <= cub)
				{
					overlap = true;
					break;
				}					
			...
			if ( overlap )
				throw new DataStructureRangeException("Overlapping child range.");
			
			DataStructureRange r =
				new DataStructureRange(dsName, this, clb, cub);
			...
			throws DataStructureRangeException
		{
			...
			if ( overlap )
				throw new DataStructureRangeException("Overlapping child value.");
			
			DataStructureRange r =
				new DataStructureRange(dsName, this, kv);
	...
	class DataStructureContentProvider implements ITreeContentProvider {
		...
		public boolean hasChildren(Object element) {
			LinkedList<DataStructureRange> c =
				((DataStructureRange) element).getChildren();
	...
	class DataStructureLabelProvider implements ILabelProvider {
		...
		public String getText(Object element) {
			...
			if (dsr.getIsWholeRange())
				r = dsr.getName();
	
			else if (dsr.getIsSingleValue()) {
				if ( dsr.getParent() == null )
					r = dsr.getName();
				else
					r = "<" + dsr.getValue().toString() + ">";
			}
