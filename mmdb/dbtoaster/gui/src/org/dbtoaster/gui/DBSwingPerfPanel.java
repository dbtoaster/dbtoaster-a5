...
public class DBSwingPerfPanel extends JPanel {
	...
	class StatPanel {
		...
		public Rectangle2D dlb;
		public Rectangle2D xb;
		public Rectangle2D yb;
		
		public StatPanel() {
			vis = new Visualization();
			display = new Display(vis);
			dlb = new Rectangle2D.Double();
			xb = new Rectangle2D.Double();
			yb = new Rectangle2D.Double();
		}
		
		public StatPanel(Visualization vis, Display display,
				Rectangle2D dlb, Rectangle2D xb, Rectangle2D yb)
		{
			this.vis = vis;
	...
	public DBPerfPanel() {
		...
		cpuPanel = new StatPanel();
		memPanel = new StatPanel();

	    //cpuPanel.display.setSize(500,400);
	    //cpuPanel.display.pan(50, 50);
	    ...
	    //memPanel.display.setSize(500,400);
	    //memPanel.display.pan(50, 50);
	    ...
	    setupPlot(cpuPanel, "t", "cpu usage", "cpu");
	    setupPlot(memPanel, "t", "mem usage", "mem");
	    ...
	    cpuPanel.vis.run("draw");
	    memPanel.vis.run("draw");
		}

	private void setupPlot(StatPanel p, String xfield, String yfield, String sfield)
	{
	    // --------------------------------------------------------------------
	    // STEP 1: setup the visualized data
		Table t = new Table();
		t.addColumn(xfield, double.class);
		t.addColumn(yfield, double.class);
		t.addColumn(sfield, double.class);
		for (int i=0; i < 100; ++i) {
			t.addRow();
			t.setDouble(i, 0, i*0.2);
			t.setDouble(i, 1, 10+i*Math.random());
			t.setDouble(i, 2, 20+i*Math.random());
		}
			
		p.vis.addTable(group, t);
	    
	    p.vis.setRendererFactory(new RendererFactory() {
	    ...
	    p.vis.putAction("update", update);

	    p.display.setBorder(BorderFactory.createEmptyBorder(10,10,10,10));
	    p.display.setSize(700,450);
	    p.display.setHighQuality(true);
	    p.display.addComponentListener(new ComponentAdapter() {
	        public void componentResized(ComponentEvent e) {
	            System.out.println("Resizing");
	            displayLayout(cpuPanel);
	            displayLayout(memPanel);
	        }
	    });
	    displayLayout(cpuPanel);
	    displayLayout(memPanel);
	...
	public void displayLayout(StatPanel p) {
	    Insets i = p.display.getInsets();
	    int w = p.display.getWidth();
	    int h = p.display.getHeight();
	    ...
	    p.dlb.setRect(i.left, i.top, w-iw-aw, h-ih-ah);
	    p.xb.setRect(i.left, h-ah-i.bottom, w-iw-aw, ah-10);
	    p.yb.setRect(i.left, i.top, w-iw, h-ih-ah);
