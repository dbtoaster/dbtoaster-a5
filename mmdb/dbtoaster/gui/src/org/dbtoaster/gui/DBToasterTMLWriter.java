...
import org.dbtoaster.gui.DatasetManager.Dataset;
...
public class DBToasterTMLWriter
{
	...
	private void init()
	{
		...
		for (String s : aggFns) aggregateFunctions.add(s);
		...
		String[] mapExprNodes = {
			"meterm", "sum", "minus", "product", "min", "max", "mapaggregate" };
		
		String[] planNodes = {
			"relation", "select", "project", "union", "cross" };
		
		for (String s : exprNodes) exprNodeTypes.add(s);
		for (String s : btermNodes) btermNodeTypes.add(s);
		for (String s : boolExprNodes) boolExprNodeTypes.add(s);
		for (String s : mapExprNodes) mapExprNodeTypes.add(s);
		for (String s : planNodes) planNodeTypes.add(s);
	}
		...
		throws CreateTMLException
	{
		...
		if ( vType == PrimitiveType.INTEGER ||
				vType == PrimitiveType.SMALLINT )
		{
			r = "int";
		}
	...
	private LinkedList<QuerySelect> getSelectQueries(
			QueryValueExpression qve)
		throws DBToasterUnhandledException
	{
		LinkedList<QuerySelect> r = new LinkedList<QuerySelect>();
		...
		// Subqueries
			...
			ValueExpressionScalarSelect scalarSelect =
				(ValueExpressionScalarSelect) qve;
		...
		// Cast clauses
			...
			throw new DBToasterUnhandledException("Unsupported: ValueExpressionVariable.");
		}
		...
		throws DBToasterUnhandledException
	{
		...
		if ( qsc instanceof SearchConditionCombined ) {
			...
			try {
				...
				Method getSelectQueriesMethod =
					this.getClass().getDeclaredMethod(
						"getSelectQueries", getDispatchClass(qsc));
	...
	private LinkedList<Function>
	getAggregates(LinkedList<QuerySelect> selects)
		throws DBToasterUnhandledException
	{
		...
		for (QuerySelect qs : selects)
		{
			...
			for (; selColIt.hasNext(); ) {
				QueryResultSpecification qsr =
					(QueryResultSpecification) selColIt.next();
		...
		throws DBToasterUnhandledException
	{
		...
		if ( qve instanceof ValueExpressionFunction )
		{
			...
			if ( !(f.isColumnFunction() &&
					aggregateFunctions.contains(f.getName().toLowerCase())) )
			{
	...
	private String createExpressionNode(String nodeType) throws CreateTMLException
	{
	...
	private String createBoolExprNode(String nodeType) throws CreateTMLException
	{
		if ( !boolExprNodeTypes.contains(nodeType) )
			throw new CreateTMLException("Invalid boolexpression node: " + nodeType);

		return "<attribute name=\"boolexpression\" value=\"" + nodeType + "\"/>";			
	}
	
	private String createMapExpressionNode(String nodeType) throws CreateTMLException
	{
		if ( !mapExprNodeTypes.contains(nodeType) )
			throw new CreateTMLException("Invalid mapexpression node: " + nodeType);
	...
	// TODO: error handling for when we cant create a valid map expression.
		...
		throws CreateTMLException
	{
		...
		QuerySelect select =
			(QuerySelect) selectStmt.getQueryExpr().getQuery();
		...
		throws CreateTMLException
	{
		...
		if ( !select.getGroupByClause().isEmpty() ) {
			throw new CreateTMLException("GROUP BY unsupported.");
		}
		...
		LinkedList<String> fromTML = createFromClauseTreeML(select.getFromClause());
		
		System.out.println("Generating whereTML: " + select.isSetWhereClause()
				+ " (null: " + (select.getWhereClause() == null) + ")");
		
		LinkedList<String> whereTML =
			((select.getWhereClause() != null && select.isSetWhereClause())?
				createWhereClauseTreeML(select.getWhereClause(), fromTML) :
				fromTML);

		System.out.println("Generating selectListTML.");

		LinkedList<String> aggregateTML =
			createSelectListTreeML(select.getSelectClause(), whereTML);
	...
	private LinkedList<String> createSelectListTreeML(
			EList selectList, LinkedList<String> relationalTML)
		throws CreateTMLException
	{
		...
		for(Iterator selIt = selectList.iterator(); selIt.hasNext();)
		{
			...
			if ( nextResult instanceof ResultTableAllColumns ) {
				throw new CreateTMLException("SELECT Table.* queries unsupported");
			}
			...
			r.addAll(createQueryValueExpressionTreeML(colValExpr, relationalTML, true));
		}
		...
		throws CreateTMLException
	{
		...
		if ( qve instanceof ValueExpressionColumn ) {
			...
			if ( mapExpression ) {
				...
				r.addAll(createMETerm("attr", column.getName()));
			} else {
				r.add(createExpressionNode("eterm"));
				r.addAll(createETerm("attr", column.getName()));
			}
			...
			r = createCombinedOperatorTreeML(
				combined.getCombinedOperator(), left, right, mapExpression);
		}
		...
		// TODO ...
			...
			throw new CreateTMLException("Unsupported: null values in select list.");
		}
		...
		// Cast clauses
			...
			throw new CreateTMLException("Unsupported: ValueExpressionVariable.");
		}

		// Unclear what these are
		else if ( qve instanceof ValueExpressionDefaultValue ) {
			throw new CreateTMLException("Unsupported: ValueExpressionDefaultValue.");
		}
	...
	private LinkedList<String> createCombinedOperatorTreeML(
			ValueExpressionCombinedOperator op,
			LinkedList<String> left, LinkedList<String> right, boolean mapExpression)
		throws CreateTMLException
	{
		...
		throws CreateTMLException
	{
		...
		// -- query should include parsing from clause, and where clause first, and
		...
		QueryValueExpression f =
			(QueryValueExpression) func.getParameterList().get(0);
		...
		throws CreateTMLException
	{
		...
		if ( tableRef instanceof TableJoined ) {
			...
			LinkedList<String> crossTML = createCrossProductTreeML(leftTML, rightTML);
		...
		throws CreateTMLException
	{
		...
		else if ( !(dbTable instanceof TemporaryTable) )
		{
			idsAndTypes = new LinkedHashMap<String, String>();
			Iterator colIt = table.getColumnList().iterator();
			for (; colIt.hasNext(); )
			{
				ValueExpressionColumn column = (ValueExpressionColumn) colIt;
	
				DataType colType = column.getDataType();
				if ( colType instanceof PredefinedDataType ) {
					PredefinedDataType sqlType = (PredefinedDataType) colType;
					
					// `ETerm or `METerm simple.getValue()
					String dbtType = getDBToasterType(sqlType.getPrimitiveType());
					
					System.out.println("TC: " + column.getName() + " " + dbtType);
					idsAndTypes.put(column.getName(), dbtType);
				}
				else {
					throw new CreateTMLException("Unsupported column type: " +
						colType.getClass().getName());
				}
			}
		}
		...
		throws CreateTMLException
	{
		...
		LinkedList<String> whereTML =
			createQuerySearchConditionTreeML(whereClause);
	...
	private LinkedList<String> createQuerySearchConditionTreeML(QuerySearchCondition sc)
		throws CreateTMLException
	{
	...
	private String predicateComparisonOpTreeML(
			PredicateComparisonOperator op, boolean mapExpression)
	{
		...
		throws CreateTMLException
	{
		...
		try {
			...
			System.out.println("Found " + nestedAggs.size() + " nested aggregates.");
		}
		...
		if ( mapExpression ) {
			...
			tmp.addAll(rightTML);
			tmp = makeBranch(tmp);
			tmp.addFirst(createBTermNode(
				predicateComparisonOpTreeML(p.getComparisonOperator(),
					mapExpression)));
	...
	private LinkedList<String> createSearchConditionCombinedTreeML(
			SearchConditionCombined pred)
		throws CreateTMLException
	{
		System.out.println("SC: " + pred);
		
		LinkedList<String> r = new LinkedList<String>();
		r.add(createSearchConditionCombinedOperatorTreeML(
			pred.getCombinedOperator()));
		...
		if ( pred.getRightCondition() != null ) {
			rightTML = createQuerySearchConditionTreeML(pred.getRightCondition());
		}
	...
	// TODO:
	private LinkedList<String> predicateTML(PredicateBetween p) throws CreateTMLException
	{
		throw new CreateTMLException("Unsupported: PredicateBetween");
	}
	
	private LinkedList<String> predicateTML(PredicateExists p) throws CreateTMLException
	{
		throw new CreateTMLException("Unsupported: PredicateExists.");
	}
	
	private LinkedList<String> predicateTML(PredicateIn p) throws CreateTMLException
	{
		throw new CreateTMLException("Unsupported: PredicateIn");
	}
	
	private LinkedList<String> predicateTML(PredicateInValueList p) throws CreateTMLException
	{
	...
	private LinkedList<String> predicateTML(PredicateIsNull p) throws CreateTMLException {
		throw new CreateTMLException("Unsupported: PredicateIsNull");
	}
	
	private LinkedList<String> predicateTML(PredicateLike p) throws CreateTMLException {
	...
	private LinkedList<String> predicateTML(PredicateQuantifiedRowSelect p)
		throws CreateTMLException
	{
		throw new CreateTMLException("Unsupported: PredicateQuantifiedRowSelect");
	}
	...
	private LinkedList<String> predicateTML(PredicateQuantifiedValueSelect p)
		throws CreateTMLException
	{
		throw new CreateTMLException("Unsupported: PredicateQuantifiedValueSelect");
	}
