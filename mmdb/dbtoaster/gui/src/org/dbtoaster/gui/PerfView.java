...
public class PerfView extends ViewPart {
	...
	public void createPartControl(Composite parent) {
		...
		for (int i=0; i < numDatabases; ++i) {
			...
			framecomp.setLayoutData(frameData);
			
			Frame dbframe = SWT_AWT.new_Frame(framecomp);
			dbframe.setLayout(new BorderLayout());
			Panel dbPanel = new Panel();
			dbframe.add(dbPanel, BorderLayout.CENTER);
			JRootPane dbroot = new JRootPane();
			...
			*/
			...
			if ( !rightLabelAdded ) {
				...
				GridData yMemData = new GridData(SWT.FILL, SWT.CENTER, false, true);
		...
		perfStatus.setBackground(
			d.getSystemColor(SWT.COLOR_TITLE_INACTIVE_BACKGROUND));
		perfStatus.setForeground(
			d.getSystemColor(SWT.COLOR_TITLE_INACTIVE_FOREGROUND));
