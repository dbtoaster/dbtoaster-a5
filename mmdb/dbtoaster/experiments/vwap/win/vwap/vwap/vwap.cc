...
#line 1 "C:/Program Files (x86)/PostgreSQL/8.3/include/sqlca.h"
...
#line 351 "vwap.cpp"
	...
	{ ECPGdo(__LINE__, 0, 1, NULL, 0, 2, cmds_str, ECPGt_EOIT, ECPGt_EORT);
#line 353 "vwap.cpp"
...
#line 353 "vwap.cpp"
	...
	scriptSw->Stop();
	Console::WriteLine("{0} s for 'VWAP matview setup'",
...
#line 368 "vwap.cpp"
	...
	{ ECPGdo(__LINE__, 0, 1, NULL, 0, ECPGst_normal, "select  avg ( price * volume )  from vwap   ", ECPGt_EOIT, 
	ECPGt_double,&(vwap),(long)1,(long)1,sizeof(double), 
	ECPGt_int,&(vwap_ind),(long)1,(long)1,sizeof(int), ECPGt_EORT);
#line 372 "vwap.cpp"
...
#line 374 "vwap.cpp"
	...
	querySw->Stop();
	Console::WriteLine("{0} s for 'VWAP matview iter'",
		querySw->Elapsed.TotalSeconds);

	return make_tuple(vwap_ind == 0, vwap);
}

void vwap_pg_triggers(string sql_file, string directory, long query_freq,
	stream^ s, ofstream* results, ofstream* log, bool dump = false)
{
	order_book bids;
	order_book asks;

	{ ECPGconnect(__LINE__, 0, "orderbook" , "yanif" , "abc" , NULL, 0); 
#line 389 "vwap.cpp"
...
#line 397 "vwap.cpp"
	...
	{
		Stopwatch^ tupleSw = Stopwatch::StartNew();
		...
		if (action == "B") {
			// Insert bids
			bids[order_id] = input_tuple(ts, p, v);

			{ ECPGdo(__LINE__, 0, 1, NULL, 0, ECPGst_normal, "insert into bids values (  $1  ,  $2  ,  $3  ,  $4  ) ", 
	ECPGt_unsigned_long_long,&(ts),(long)1,(long)1,sizeof(unsigned long long), 
	ECPGt_NO_INDICATOR, NULL , 0L, 0L, 0L, 
	ECPGt_int,&(order_id),(long)1,(long)1,sizeof(int), 
	ECPGt_NO_INDICATOR, NULL , 0L, 0L, 0L, 
	ECPGt_double,&(p),(long)1,(long)1,sizeof(double), 
	ECPGt_NO_INDICATOR, NULL , 0L, 0L, 0L, 
	ECPGt_double,&(v),(long)1,(long)1,sizeof(double), 
...
#line 432 "vwap.cpp"
			...
			// Insert asks
			asks[order_id] = input_tuple(ts, p, v);

			{ ECPGdo(__LINE__, 0, 1, NULL, 0, ECPGst_normal, "insert into asks values (  $1  ,  $2  ,  $3  ,  $4  ) ", 
	ECPGt_unsigned_long_long,&(ts),(long)1,(long)1,sizeof(unsigned long long), 
	ECPGt_NO_INDICATOR, NULL , 0L, 0L, 0L, 
	ECPGt_int,&(order_id),(long)1,(long)1,sizeof(int), 
	ECPGt_NO_INDICATOR, NULL , 0L, 0L, 0L, 
...
#line 438 "vwap.cpp"
		...
		else if (action == "E")
		{
			...
			if ( found != bids.end() ) {
				unsigned long long order_ts = found->second->ts;
				double order_p = found->second->price;
				double order_v = found->second->volume;
				new_volume = order_v - v;

				bids[order_id] = input_tuple(order_ts, order_p, new_volume);

				{ ECPGdo(__LINE__, 0, 1, NULL, 0, ECPGst_normal, "update bids set volume  =  $1   where id =  $2  ", 
	ECPGt_double,&(new_volume),(long)1,(long)1,sizeof(double), 
	ECPGt_NO_INDICATOR, NULL , 0L, 0L, 0L, 
	ECPGt_int,&(order_id),(long)1,(long)1,sizeof(int), 
	ECPGt_NO_INDICATOR, NULL , 0L, 0L, 0L, ECPGt_EOIT, ECPGt_EORT);
#line 453 "vwap.cpp"
...
#line 453 "vwap.cpp"
			...
			else
			{
				...
				if ( found != asks.end() ) {
					...
					asks[order_id] = input_tuple(order_ts, order_p, new_volume);

					{ ECPGdo(__LINE__, 0, 1, NULL, 0, ECPGst_normal, "update asks set volume  =  $1   where id =  $2  ", 
	ECPGt_double,&(new_volume),(long)1,(long)1,sizeof(double), 
	ECPGt_NO_INDICATOR, NULL , 0L, 0L, 0L, 
	ECPGt_int,&(order_id),(long)1,(long)1,sizeof(int), 
	ECPGt_NO_INDICATOR, NULL , 0L, 0L, 0L, ECPGt_EOIT, ECPGt_EORT);
...
#line 468 "vwap.cpp"
		...
		else if (action == "F")
		{
			...
			if ( found != bids.end() )
			{
				{ ECPGdo(__LINE__, 0, 1, NULL, 0, ECPGst_normal, "delete from bids  where id =  $1  ", 
	ECPGt_int,&(order_id),(long)1,(long)1,sizeof(int), 
	ECPGt_NO_INDICATOR, NULL , 0L, 0L, 0L, ECPGt_EOIT, ECPGt_EORT);
#line 483 "vwap.cpp"
...
#line 483 "vwap.cpp"
				...
				if ( found != asks.end() )
				{
					{ ECPGdo(__LINE__, 0, 1, NULL, 0, ECPGst_normal, "delete from asks  where id =  $1  ", 
	ECPGt_int,&(order_id),(long)1,(long)1,sizeof(int), 
	ECPGt_NO_INDICATOR, NULL , 0L, 0L, 0L, ECPGt_EOIT, ECPGt_EORT);
#line 492 "vwap.cpp"
		...
		***/
		...
		if ( (tuple_counter % 10000) == 0 )
		{
			...
			cout << "Exec time " << (tup_sum / 10000.0) << " "
				<< (tup10kSw->Elapsed.TotalSeconds) << endl;
...
int main(array<String ^> ^args)
{
	...
	if ( app_mode == "toasted" || app_mode == "triggers" )
	{
	...
	else if ( app_mode == "pg_triggers" )
	{
		Console::WriteLine("Running VWAP postgres.");

		marshal_context ^ context = gcnew marshal_context();
		const char* log_file_name_cstr = context->marshal_as<const char*>(log_file_name);
		puts(log_file_name_cstr);

		const char* results_file_name_cstr = context->marshal_as<const char*>(results_file_name);
		puts(results_file_name_cstr);

		const char* stats_file_name_cstr = context->marshal_as<const char*>(stats_file_name);
		puts(stats_file_name_cstr);

	    const char* script_file_name_cstr = context->marshal_as<const char*>(script_file_name);
     	puts(script_file_name_cstr);

		const char* directory_cstr = context->marshal_as<const char*>(directory);
     	puts(directory_cstr);

		string s(script_file_name_cstr);
		string d(directory_cstr);
	    delete context;

		ofstream* log = new ofstream(log_file_name_cstr);
		ofstream* out = new ofstream(results_file_name_cstr);
		ofstream* stats = new ofstream(stats_file_name_cstr);

		cout << "Using script file " << s << endl;
		vwap_pg_triggers(s, d, query_freq, f, out, log);
